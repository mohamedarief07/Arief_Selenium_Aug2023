...
public class LearnDatePicker2 {
	...
	public void pickTheDate() {
		...
		for (WebElement list2 : allDate) {
			...
			// date01
			List<WebElement> allDate1 = baseTable
					.findElements(By.xpath("(//table[@class='table-condensed'])[1]/tbody/tr/following-sibling::tr/td"));

			for (WebElement list3 : allDate1) {
				String text3 = list3.getText();

				System.out.println("Date :" + text3);
				if (text3.equalsIgnoreCase(date)) {
					if (list3.isEnabled()) {
						list3.click();
						break;
